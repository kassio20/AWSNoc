...
class DatabaseManager:
    ...
    def get_accounts(self) -> List[Dict]:
        ...
        try:
            ...
            for row in cursor.fetchall():
                account = dict(row)
                account['services'] = account['services'] if isinstance(account['services'], list) else (json.loads(account['services']) if account['services'] else [])
    ...
    def get_resources(self, account_id: int) -> List[Dict]:
        ...
        try:
            ...
            for row in cursor.fetchall():
                resource = dict(row)
                resource['metadata'] = resource['metadata'] if isinstance(resource['metadata'], dict) else (json.loads(resource['metadata']) if resource['metadata'] else {})
    ...
    def get_alerts(self, account_id: Optional[int] = None, active_only: bool = True) -> List[Dict]:
        ...
        try:
            ...
            for row in cursor.fetchall():
                alert = dict(row)
                alert['ai_analysis'] = alert['ai_analysis'] if isinstance(alert['ai_analysis'], dict) else (json.loads(alert['ai_analysis']) if alert['ai_analysis'] else {})
                alerts.append(alert)
